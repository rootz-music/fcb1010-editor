...
def main():
    ...
    if not os.path.exists("credentials.json"):
        ...
        print(
            "Please download it from the Google API Console and place it in the current directory."
        )
        return
    ...
    if command == "export":
        if len(sys.argv) < 4:
            print(
                "Usage: python sheets_integration.py export <json_file> <spreadsheet_name>"
            )
            return
        ...
        if sheets.export_presets_to_sheet(presets_data, worksheet):
            print(
                f"Successfully exported {len(presets_data)} presets to '{spreadsheet_name}'"
            )

    elif command == "import":
        if len(sys.argv) < 4:
            print(
                "Usage: python sheets_integration.py import <spreadsheet_name> <json_file>"
            )
            return
