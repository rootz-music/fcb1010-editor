...
class PresetEditor:
    ...
    def save_presets(self, filename=None):
        ...
        if not target_file:
            logger.error("No filename specified for saving presets.")
            return False
            ...
            if choice == "1":
                ...
                try:
                    ...
                    if (
                        ...
                        self.modified = True
                        print(
                            f"Added control change: {controller}={value} on channel {channel}"
                        )
                    else:
                        print(
                            "Invalid values. Controller and value must be 0-127, channel 0-15."
                        )
                except ValueError:
                    print("Please enter valid numbers")
            ...
            if choice == "1":
                ...
                try:
                    idx = (
                        int(
                            input(
                                f"Enter index to edit (1-{len(preset.program_changes)}): "
                            )
                        )
                        - 1
                    )
                    if 0 <= idx < len(preset.program_changes):
                        ...
                        program = input(
                            f"Enter program number (0-127) [{pc['program']}]: "
                        )
                        ...
                        self.modified = True
                        print(
                            f"Updated program change: {pc['program']} on channel {pc['channel']}"
                        )
                    else:
                        print("Invalid index")
                except ValueError:
                ...
                try:
                    idx = (
                        int(
                            input(
                                f"Enter index to delete (1-{len(preset.program_changes)}): "
                            )
                        )
                        - 1
                    )
        ...
        while True:
            ...
            if not preset.control_changes:
                ...
                for i, cc in enumerate(preset.control_changes):
                    print(
                        f"{i+1}. Controller: {cc['controller']}, Value: {cc['value']}, Channel: {cc['channel']}"
                    )
            ...
            if choice == "1":
                ...
                try:
                    idx = (
                        int(
                            input(
                                f"Enter index to edit (1-{len(preset.control_changes)}): "
                            )
                        )
                        - 1
                    )
                    if 0 <= idx < len(preset.control_changes):
                        ...
                        controller = input(
                            f"Enter controller number (0-127) [{cc['controller']}]: "
                        )
                        ...
                        self.modified = True
                        print(
                            f"Updated control change: {cc['controller']}={cc['value']} on channel {cc['channel']}"
                        )
                    else:
                        print("Invalid index")
                except ValueError:
                ...
                try:
                    idx = (
                        int(
                            input(
                                f"Enter index to delete (1-{len(preset.control_changes)}): "
                            )
                        )
                        - 1
                    )
                    ...
                    if choice.lower() == "n" and page < total_pages - 1:
                        ...
                        try:
                            idx = (
                                int(
                                    input(
                                        f"Enter preset number (1-{len(self.presets)}): "
                                    )
                                )
                                - 1
                            )
        ...
        try:
            ...
            if 0 <= idx < len(self.presets):
                ...
                for cc in preset.control_changes:
                    self.fcb.send_control_change(
                        cc["controller"], cc["value"], cc["channel"]
                    )
        ...
        while True:
            ...
            if choice == "1":
                ...
                if filename:
                    if self.modified:
                        save = input(
                            "Current presets have been modified. Save first? (y/n): "
                        )
                        if save.lower() == "y":
                            self.save_presets()
                ...
                if self.modified:
                    save = input(
                        "Presets have been modified. Save before quitting? (y/n): "
                    )
                    if save.lower() == "y":
                        self.save_presets()
