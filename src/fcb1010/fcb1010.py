...
class FCB1010:
    ...
    def __init__(self, input_port=None, output_port=None):
        ...
        if input_port is not None and 0 <= input_port < len(available_in_ports):
            ...
            logger.info(f"Connected to MIDI Input port: {available_in_ports[input_port]}")
        elif available_in_ports:
            # Find a port containing "FCB1010" if possible
            fcb_ports = [i for i, port in enumerate(available_in_ports) if "FCB1010" in port]
            if fcb_ports:
                self.midi_in.open_port(fcb_ports[0])
                logger.info(f"Connected to FCB1010 MIDI Input port: {available_in_ports[fcb_ports[0]]}")
            else:
                self.midi_in.open_port(0)
                logger.info(f"Connected to default MIDI Input port: {available_in_ports[0]}")
        else:
        ...
        if output_port is not None and 0 <= output_port < len(available_out_ports):
            ...
            logger.info(f"Connected to MIDI Output port: {available_out_ports[output_port]}")
        elif available_out_ports:
            fcb_ports = [i for i, port in enumerate(available_out_ports) if "FCB1010" in port]
            if fcb_ports:
                self.midi_out.open_port(fcb_ports[0])
                logger.info(f"Connected to FCB1010 MIDI Output port: {available_out_ports[fcb_ports[0]]}")
            else:
                self.midi_out.open_port(0)
                logger.info(f"Connected to default MIDI Output port: {available_out_ports[0]}")
        else:
    ...
    def send_program_change(self, program_number, channel=0):
        ...
        if 0 <= program_number <= 127 and 0 <= channel <= 15:
            ...
            logger.error(f"Invalid program number ({program_number}) or channel ({channel})")
    ...
    def send_control_change(self, controller, value, channel=0):
        ...
        if (0 <= controller <= 127 and 0 <= value <= 127 and 0 <= channel <= 15):
            ...
            self.midi_out.send_message(message)
            logger.info(f"Sent Control Change: controller={controller}, value={value}, channel={channel}")
        else:
            logger.error(f"Invalid controller ({controller}), value ({value}), or channel ({channel})")
...
class Preset:
    ...
    def add_control_change(self, controller, value, channel=0):
        ...
        Args:
            ...
            channel (int, optional): MIDI channel (0-15). Defaults to 0.
        """
        self.control_changes.append({"controller": controller, "value": value, "channel": channel})
    ...
    def from_dict(cls, data):
        ...
        for cc in data.get("control_changes", []):
            preset.add_control_change(cc["controller"], cc["value"], cc.get("channel", 0))
